...
class ToolsPlugin(Plugin, ToolProvider):
    ...
    def start(self) -> None:
        ...
        print(f"[Tools] Initialized, exec={'enabled' if self._exec_enabled else 'disabled'}", file=sys.stderr)
    ...
    def _wallet_pay(self, invoice: str) -> str:
        ...
        try:
            ...
            return "Payment successful" if result.get("success") else f"Failed: {result.get('error')}"
        except Exception as e:
            return f"Error: {e}"
