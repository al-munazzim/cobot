...
class PluginRegistry:
    ...
    def __init__(self):
        ...
        self._capabilities: dict[str, list[str]] = defaultdict(list)  # capability -> [ids]
    ...
    def register(self, plugin_class: Type[Plugin]) -> Plugin:
        ...
        if not isinstance(plugin_class, type) or not issubclass(plugin_class, Plugin):
            raise PluginError(f"Invalid plugin: {plugin_class} is not a Plugin subclass")
        
        # Check for meta attribute
        if not hasattr(plugin_class, 'meta') or not isinstance(plugin_class.meta, PluginMeta):
            raise PluginError(f"Plugin {plugin_class.__name__} missing valid 'meta' attribute")
    ...
    def configure_all(self, config: dict) -> None:
        ...
        for plugin_id in self._load_order:
            ...
            try:
                ...
                print(f"[Registry] Failed to configure '{plugin_id}': {e}", file=sys.stderr)
    ...
    def run_hook(self, hook_name: str, ctx: dict) -> dict:
        ...
        for plugin_id in self._load_order:
            ...
            try:
                ...
                print(f"[Registry] Error in {plugin_id}.{hook_name}: {e}", file=sys.stderr)
                if hook_name != "on_error":
                    self.run_hook("on_error", {
                        "error": e,
                        "hook": hook_name,
                        "plugin": plugin_id,
                    })
