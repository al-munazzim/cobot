...
class Cobot:
    ...
    def respond(self, message: str, sender: str = "unknown") -> str:
        ...
        ctx = run("transform_system_prompt", {
            "prompt": self.soul,
            "peer": sender,
            "messages": messages,
        })
        ...
        try:
            for _ in range(max_rounds):
                ...
                for tool_call in response.tool_calls:
                    ...
                    ctx = run("on_before_tool_exec", {"tool": tool_name, "args": tool_args})
                    if ctx.get("abort"):
...
def main():
    ...
    if config_plugin:
        ...
        if config.provider == "ppq":
            import os
            if not config_data.get("ppq", {}).get("api_key") and not os.environ.get("PPQ_API_KEY"):
